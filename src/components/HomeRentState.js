...
const HomeRentState = () => {
  ...
  const handleSearchIconClick = () => {
    ...
    const filtered = properties.filter((property) =>
      Object.values(property).some((value) => {
        if (
          typeof value === "string" ||
          value instanceof String // Verifica si es una cadena de texto
        ) {
          return value.toLowerCase().includes(searchValue.toLowerCase());
        }
        return false; // Si no es una cadena de texto, retorna false
      })
    );
  ...
  useEffect(() => {
    // Filtra las propiedades marcadas como "rented"
    const filteredNotRented = properties.filter(
      (property) => property.rent !== "rented"
    );
    setFilteredProperties(filteredNotRented);
  }, []); // Ejecuta esto solo una vez al cargar el componente
